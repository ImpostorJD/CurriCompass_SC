...
class InitSample extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:init-sample {--no-course-availability=false : Specify if course availability should be true or false} {--no-course=false : Specify if course should be true or false} {--no-student-record=false : Specify if students should be true or false} {--no-program=false : Specify if no program should be generated} {--no-curriculum=false : Specify if no curriculum should be generated} {--no-school-year=false : Specify if no school year should be generated}';
    ...
    public function handle()
    {
        ...
        $no_course = $this->option('no-course') === 'true';
        ...
        #needs course to proceed
        if(!$no_course){
            $this->call('db:seed', ['--class' => 'CourseSeeder']);
            echo("Course/Subjects Table successfully seeded.\n");
        }

        #needs course, curriculum, school year and program to proceed
        if(!$no_course && !$no_school_year && !$no_program && !$no_curriculum){
            $this->call('db:seed', ['--class' => 'curricula_seeder']);
            echo("Curricula Table successfully seeded.\n");
        }

        #needs course, school_year and course availability to proceed
        if(!$no_course && !$no_school_year && !$no_course_availability){
        ...
        if(!$no_course && !$no_school_year && !$no_program && !$no_curriculum && !$no_student_record){
